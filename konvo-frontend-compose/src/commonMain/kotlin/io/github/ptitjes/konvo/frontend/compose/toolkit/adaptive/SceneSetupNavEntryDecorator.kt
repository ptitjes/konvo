...
@Composable
fun rememberSceneSetupNavEntryDecorator(): NavEntryDecorator<Any> = remember {
...
fun SceneSetupNavEntryDecorator(): NavEntryDecorator<Any> {
    val movableContentMap: MutableMap<Any, @Composable (@Composable () -> Unit) -> Unit> =
        mutableStateMapOf()
    return navEntryDecorator(onPop = { contentKey -> movableContentMap.remove(contentKey) }) {
            entry,
        ->
 ...
 */
val LocalEntriesToRenderInCurrentScene: ProvidableCompositionLocal<Set<Any>> =
    compositionLocalOf {
